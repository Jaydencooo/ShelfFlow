...
export async function PUT(
  ...
  try {
    ...
    const rule = await gatewayRequest<AdminPricingRule>(`/api/admin/pricing-rules/${id}`, {
      method: "PUT",
      body: payload,
      accessToken
    })

    return NextResponse.json(buildSuccessResponse(rule, requestId, "定价规则更新成功"))
  } catch (error) {
...
export async function DELETE(
  ...
  try {
    const accessToken = await requireAccessToken()
    const { id } = await context.params
    await gatewayRequest(`/api/admin/pricing-rules/${id}`, {
      method: "DELETE",
      accessToken
    })

    return NextResponse.json(buildSuccessResponse(null, requestId, "定价规则已删除"))
  } catch (error) {
    const appError = error instanceof AppError
      ? error
      : new AppError(errorCodes.DEPENDENCY_ERROR, 502, "定价规则删除失败")
