...
import type { AdminAiOpsSuggestionAction } from "@/lib/types"
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const actions = await gatewayRequest<AdminAiOpsSuggestionAction[]>("/api/admin/ai-ops/suggestions/actions", { accessToken })
    return NextResponse.json(buildSuccessResponse(actions, requestId))
  } catch (error) {
    const appError = error instanceof AppError
      ? error
      : new AppError(errorCodes.DEPENDENCY_ERROR, 502, "运营建议执行记录查询失败")
