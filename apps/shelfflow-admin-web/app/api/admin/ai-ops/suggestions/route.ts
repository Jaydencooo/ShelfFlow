...
import type { AdminAiOpsSuggestion } from "@/lib/types"
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const suggestions = await gatewayRequest<AdminAiOpsSuggestion[]>("/api/admin/ai-ops/suggestions", { accessToken })
    return NextResponse.json(buildSuccessResponse(suggestions, requestId))
  } catch (error) {
    const appError = error instanceof AppError
      ? error
      : new AppError(errorCodes.DEPENDENCY_ERROR, 502, "AI 运营建议查询失败")
