...
export async function GET(request: Request) {
  ...
  try {
    ...
    const operationLogs = await gatewayRequest<AdminOperationLog[]>(`/api/admin/operation-logs?limit=${encodeURIComponent(limit)}`, { accessToken })
    return NextResponse.json(buildSuccessResponse(operationLogs, requestId))
  } catch (error) {
    const appError = error instanceof AppError
      ? error
      : new AppError(errorCodes.DEPENDENCY_ERROR, 502, "操作日志查询失败")
