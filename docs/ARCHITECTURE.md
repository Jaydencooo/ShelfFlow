...
用户端商品目录和分类是高频读接口，可通过 `USER_CATALOG_CACHE_ENABLED=true` 开启 Redis 缓存。

- 缓存范围：用户端商品分类列表、商品分页结果。
- Key 规范：统一使用 `STOREFRONT_CATALOG_CACHE_REDIS_KEY_PREFIX`，默认 `shelfflow:storefront:catalog`。
- 失效入口：管理端商品、分类、批次、定价规则发生创建、更新、删除或状态变化时，可通过 `ADMIN_STOREFRONT_CACHE_INVALIDATION_ENABLED=true` 清理前台目录缓存。
- 降级策略：默认 `fail-open=true`，Redis 读取或清理失败只记录日志，主业务继续走数据库，避免缓存组件影响交易链路。

这层缓存解决的是用户首页和商品目录的读放大问题，同时通过管理端统一失效保证“起售/停售、库存批次、价格规则”能尽快同步到用户端。

## 9. 订单事件驱动扩展

用户端订单服务在写入 `order_event_log` 后，可通过 `USER_ORDER_EVENTS_ENABLED=true` 开启 RabbitMQ 事件发布；需要可靠投递时可再开启 `USER_ORDER_EVENTS_OUTBOX_ENABLED=true`，将事件先写入 outbox 表，由后台任务按批投递；管理端可通过 `ADMIN_ORDER_EVENTS_CONSUMER_ENABLED=true` 开启 inbox 消费。

- 发布时机：注册事务同步，在订单事务提交后发布，避免回滚订单产生脏事件。
- 可靠投递：outbox 表按 `event_id` 唯一约束去重，支持批量扫描、失败重试和投递成功标记。
- 事件交换机：默认 `shelfflow.order.events`，通过 `USER_ORDER_EVENTS_EXCHANGE` 调整。
- 路由键：默认 `shelfflow.order.{eventType}`，例如 `shelfflow.order.submitted`、`shelfflow.order.paid`。
- 消费队列：默认 `shelfflow.admin.order-events.inbox`，通过 `ADMIN_ORDER_EVENTS_CONSUMER_QUEUE` 调整。
- 幂等消费：管理端 inbox 表按 `event_id` 唯一约束去重，重复投递直接复用已处理记录。
- 失败策略：默认只记录日志，不影响用户下单；需要强一致演示时可设置 `USER_ORDER_EVENTS_FAIL_FAST=true`。

这层能力让订单链路具备可追踪的异步扩展点，后续可以平滑接入短信/邮件通知、履约排队、经营分析实时聚合和风控审计。

## 10. 支付幂等模型

用户端支付接口保持 `POST /api/user/orders/{id}/pay` 不变，后端新增 `user_order_payment` 支付单承接幂等语义，并提供可配置的模拟支付平台回调入口 `POST /api/user/payment-callbacks/mock`。

- 幂等粒度：按 `userId + orderId` 生成订单级幂等键。
- 支付流水：按订单号生成稳定支付流水号，避免重复点击产生多条支付记录。
- 状态推进：支付单从 `pending` 更新为 `succeeded`，订单从 `pending_payment` 更新为 `to_prepare`。
- 重复请求：订单已支付时直接返回最新订单详情，不重复写支付事件，不影响前端体验。
- 回调幂等：支付回调按 `callback_event_id` 和 `external_trade_no` 去重，重复回调返回既有处理结果。
- 安全边界：回调默认关闭；开启后可要求 `X-ShelfFlow-Payment-Signature`，使用 HMAC-SHA256 校验关键字段。
- 并发保护：订单状态更新仍通过 `status = pending_payment and pay_status = unpaid` 条件更新保证原子性。

这套模型先适配本地模拟支付，后续接微信/支付宝/第三方支付回调时，可以复用支付单、外部交易号、回调事件 ID 和签名校验的结构。

## 11. 订单超时关单

用户端订单服务支持通过 `USER_ORDER_TIMEOUT_CLOSE_ENABLED=true` 开启未支付订单自动关单。

- 扫描范围：`pending_payment + unpaid` 且订单创建时间超过配置阈值。
- 默认阈值：`USER_ORDER_TIMEOUT_CLOSE_UNPAID_TIMEOUT_MINUTES=30`。
- 处理动作：原子更新订单为 `cancelled`，释放批次锁定库存，写入 system 审计事件。
- 批处理控制：通过 `USER_ORDER_TIMEOUT_CLOSE_BATCH_SIZE` 控制单轮处理数量，避免大批量历史订单影响在线服务。

超时关单和用户主动取消共用库存释放语义，但采用独立后台任务触发，避免前端或人工操作成为库存回收的唯一入口。
